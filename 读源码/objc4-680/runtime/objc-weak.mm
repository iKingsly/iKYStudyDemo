...
/** 
 * 注销已经注册的弱引用。
 * 这个被使用在党一个引用存储的内容即将被释放，然而这个指针还没有死掉 
 * 如果 referent/referrer 目前不是一个活跃的weak 指针不会启什么作用.
 *
 ...
 * @param 全局的weak_table
 * @param referent 对象.
 * @param referrer weak引用.
 */
void
weak_unregister_no_lock(weak_table_t *weak_table, id referent_id, 
                        id *referrer_id)
{
//    *  @param location weak指针的地址
//    *  @param newObj   weak指针指向的对象
//    weak指针的地址  referrer_id
    objc_object *referent = (objc_object *)referent_id;
    objc_object **referrer = (objc_object **)referrer_id;

    // weak_entry_t 中保存了所有指向该对象的 weak 指针。
    weak_entry_t *entry;
    ...
    if ((entry = weak_entry_for_referent(weak_table, referent))) {
        ...
        if (empty) {
            /// 将这个weak指针在weak_table 进行删除，并且把这个对象的weak_entry表数量－1
...
/**
 * 注册一个新的（object － weak指针）对，创建一个weak objc entry（如果还未存在的情况下）
 * @param weak_table 全局weak表.
 * @param referent 对象的指针.
 * @param referrer weak指针的地址.
 */
id 
                      ...
                      id *referrer_id, bool crashIfDeallocating)
{
    ...
    if (!referent->ISA()->hasCustomRR()) {
        ...
        if ((IMP)allowsWeakReference == _objc_msgForward) { /// 找不到这个方法
            return nil;
        }
        deallocating =
    ...
    /// 创建一个weak_entry_t对象 在表里增加这个的引用
    weak_entry_t *entry;
    if ((entry = weak_entry_for_referent(weak_table, referent))) { /// 先查看是否有存在了这个指针地址的weak_entry_t

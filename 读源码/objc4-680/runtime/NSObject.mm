...
//true/*old*/, true/*new*/, true/*crash*/

/**
 *  更新一个weak变量
 * 如果HaveOld为true，变量有一个存在的值需要清理，value可能为nil
   如果HaveNew为true，有一个新的value需要被变量引用，这个value也可能为nil
   如果CrashIfDeallocating为true，如果newObjc是一个已经释放了的对象或者newObj的类不支持weak引用
   如果CrashIfDeallocating为false，会保存nil
 */
...
storeWeak(id *location, objc_object *newObj)
{
    ...
    /// <false/*old*/, true/*new*/, false/*crash*/> init方法
    ...
    /// 取得表中oldObj 和 newObjc对应的 SideTable
 retry:
    if (HaveOld) {
        oldObj = *location;
    ...
    /// 防止弱引用机制和+初始化机制之间的死锁，确保没有弱引用的对象有一个未初始化的isa对象
    if (HaveNew  &&  newObj) {
    ...
    if (HaveNew) {
        newObj = (objc_object *)weak_register_no_lock(&newTable->weak_table, 
                                                      (id)newObj, location, 
                                                      CrashIfDeallocating);
        // weak_register_no_lock 返回nil 如果一个对象是不允许的对象
 ...
 */
/**
 *  这个方法保存了一个新的对象给一个weak变量，它会在任何一个weak变量是分派的地方使用，
 *
